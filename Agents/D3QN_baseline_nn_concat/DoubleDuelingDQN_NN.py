...
class DoubleDuelingDQN_NN(object):
    ...
    def construct_q_network(self):
        input_shape_s = (self.observation_size * self.num_frames,)
        input_shape_opp = (self.action_vect_size * self.num_frames,)

        input_layer_s = tfk.Input(shape = input_shape_s, name="input_obs")
        input_layer_opp = tfk.Input(shape = input_shape_opp, name="input_opp")

        lay1 = tfkl.Dense(self.observation_size * 2, name="fc_1")(input_layer_s)
        ...
        state_model = tfk.Model(inputs=[input_layer_s], outputs=lay3)

        lay_opp1 = tfkl.Dense(self.action_vect_size * 2, name="fc_opp1")(input_layer_opp)
        lay_opp1 = tfka.relu(lay_opp1, alpha=0.01)
        lay_opp2 = tfkl.Dense(self.action_vect_size, name="fc_opp2")(lay_opp1)
        lay_opp2 = tfka.relu(lay_opp1, alpha=0.01)
        opp_model = tfk.Model(inputs=[input_layer_opp], outputs=lay_opp2)

        combined = tfkl.concatenate([state_model.output, opp_model.output])

        lay4 = tfkl.Dense(512, name="fc_4")(combined)
        ...
        self.model = tfk.Model(inputs=[state_model.input, opp_model.input], outputs=[Q],
    ...
    def predict_move(self, state_data, opp_data):
        state_model_input = state_data.reshape(1, self.observation_size * self.num_frames)
        opp_model_input = opp_data.reshape(1, self.action_vect_size * self.num_frames)
        q_actions = self.model.predict([state_model_input,opp_model_input], batch_size = 1)     
