...
def plot_training(epoch_rewards, epoch_rewards_moving_avg, losses):
    plt.figure()
    ...
    plt.figure()
    plt.plot(range(len(epoch_rewards_moving_avg)), epoch_rewards_moving_avg)
    plt.xlabel('Episode')
...
def train(env,
    ...
    rewards, moving_avg_rewards, losses = agent.train(env,                                        
                                              iterations,
                                              save_path,
                                              num_pre_training_steps,
                                              logs_path)
    with open('D3QN_baseline_input_concat_rewards.data', 'wb') as file:
        # store the data as binary data stream
        pickle.dump(rewards, file)

    with open('D3QN_baseline_input_concat_average_rewards.data', 'wb') as file:
        # store the data as binary data stream
        pickle.dump(moving_avg_rewards, file)

    
    with open('D3QN_baseline_input_concat_losses.data', 'wb') as file:
        # store the data as binary data stream
        pickle.dump(losses, file)

    plot_training(rewards, moving_avg_rewards, losses[1:])    
