...
class DoubleDuelingDQN(AgentWithConverter):
    ...
    def my_act(self, state, reward, done=False):
        ...
        if len(self.frames) < self.num_frames:
            return 0 # Do nothing
        # Infer with the last num_frames states and opponent actions
        _input = np.concatenate((np.array(self.frames), np.array(self.all_opponent_actions[-self.num_frames:])), axis=1)
        a, _ = self.Qmain.predict_move(np.array(_input))
    ...
    def act(self, obs, reward, done):
        self.obs = obs     
        if done:
            print("number of attacks: {}".format(self.count_non_do_nothing_opp_act))
    ...
    def _batch_train(self, training_step, step):
        ...
        # Concatenate the state and the opponent action batch to be used as input for the main network
        s_opp_act_batch = [np.concatenate((state, opp_act), axis=1) for (state, opp_act) in zip(s_batch, opp_a_batch)]
        s_opp_act_batch = np.concatenate(s_opp_act_batch, axis=0)
        
        # Concatenate the next state and the opponent next action batch to be used as input for the target network
        s2_opp_act_batch = [np.concatenate((state, opp_act), axis=1) for (state, opp_act) in zip(s2_batch, opp_next_a_batch)]
        s2_opp_act_batch = np.concatenate(s2_opp_act_batch, axis=0)
        
        # Reshape frames to 1D
        input_size = self.observation_size * self.num_frames + self.action_vect_size * self.num_frames
        input_t = np.reshape(s_opp_act_batch, (self.batch_size, input_size))
        input_t_1 = np.reshape(s2_opp_act_batch, (self.batch_size, input_size))
